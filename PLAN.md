...
連續捲動式（非翻頁），單章渲染，接近章尾預載下一章。段落以 `data-offset` 標記字元位移，供書籤定位與 TTS 高亮共用。

快捷鍵：`↑↓`／`PgUp PgDn` 捲動、`← →` 上下章、`Ctrl+T` 目錄、`空白鍵` 播放/暫停朗讀、`F11` 全螢幕、`Ctrl+±` 字級。

**驗收**：3000 章的書任意跳章都在 200ms 內完成，捲動不掉幀。
