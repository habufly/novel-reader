...
### Phase 5 — 語音聽書（2.5–3 天）· 完成（線上引擎受阻）

實作前先探測環境，三個關鍵發現都與原本的假設不同。

**一、語音清單要輪詢，不能只信 `voiceschanged`**

第一次查詢 `getVoices()` 回傳 0 個語音，`voiceschanged` 也確實觸發了——但觸發當下清單仍是空的，要再等約 3–5 秒才填好。改用輪詢直到清單非空為止。若照直覺只監聽一次事件，使用者會看到「系統沒有中文語音」的錯誤提示。

實際可用：Hanhan、Yating、Zhiwei，三個 zh-TW 本機語音。

**二、`boundary` 事件對中文是準的——原本的降級方案不需要**

計畫原本假設 Windows SAPI 的 boundary 對中文不可靠，要退回句級高亮。實測 40 字觸發 25 次，`charIndex`/`charLength` 正確對到詞：鎮／南／是／一個／南方的／小／縣城／夏／末／酷暑／難耐。所以離線引擎也做了**逐詞高亮**，不必等線上引擎才有。

**三、`speechSynthesis.pause()` 在 Windows 上是壞的**

它回報 `paused=true`、`pause` 事件也照發，但聲音沒停——「暫停」期間 boundary 的 charIndex 仍從 10 前進到 14。所以暫停一律改成**取消 + 記住位置**，續讀時從記住的那個詞重新開始。boundary 給的位移很精確，續讀點聽起來是自然的。

**四、線上 Edge 神經語音目前無法使用**

語音清單（HTTPS）取得正常，322 個、含 3 個 zh-TW 神經語音；但合成用的 WebSocket 端點一律回 **403**。已排除三種可能：換過各種標頭組合與 `Sec-MS-GEC` 權杖版本都是 403；時鐘與伺服器只差 1 秒，不是時鐘偏差；WebSocket 本身沒被網路阻擋（連得上其他 WSS 伺服器），同主機的 HTTPS 也是 200。結論是微軟這個非官方端點拒絕連線。

程式碼保留完整實作與**自動降級**：合成失敗時切回離線引擎並提示，朗讀不中斷。實測切到線上引擎後，會顯示「線上語音無法使用，已自動切回離線語音」，而且高亮持續前進、沒有變成無聲。日後端點恢復即可直接使用。

**高亮用 CSS Custom Highlight API**

逐詞高亮每秒要更新好幾次。若把段落拆成 `<span>`，React 得重新算繪整段，使用者選取中的文字也會被打斷。Highlight API 只在既有文字節點上疊樣式，不動 DOM 結構。不支援時退回整段標記。

**其他**

句子切塊（40–160 字，優先在句號斷、退而求其次在逗號斷）、跨章自動連播、倍速 0.5–3.0、音調音量、睡眠計時（15/30/60 分鐘／本章結束）、朗讀位置即閱讀位置（自動書籤跟著唸到哪停到哪）、媒體鍵控制、朗讀中最小化收進系統匣。

**驗收結果**

| 項目 | 結果 |
|---|---|
| 從閱讀位置接著唸 | 開啟即從第 35 句（上次讀到處）開始 |
| 逐詞高亮 | 一次 → 老板 → 值，持續跟上語音 |
| 暫停 | 高亮凍結 3 秒不動；續讀從原詞接續 |
| 跨章連播 | 第3章 96/97 → 第4章 1/150，不中斷 |
| 線上引擎失敗 | 顯示提示、切回離線、朗讀不中斷 |
| 朗讀時書籤同步 | 磁碟位移 1234 → 1255 → 1407 → 1440 |
| 打包版 | 啟動正常，`ws` 正確打包，朗讀與高亮皆可用 |
...
先量測才知道問題出在哪。實測 Windows 語音每換一段有約 **700ms 的固定間隔，且不隨段落長短改變**（45 字段落間隔 656ms、250 字段落 750ms）。試過把後續段落預先排進佇列讓引擎自己接著唸，間隔幾乎沒變（248/688ms → 303/676ms），可見延遲來自 SAPI 本身，改不掉。

既然間隔的長度改不掉，就只能降低它出現的頻率。翻出真正的元凶：切塊器是**逐段落處理**的，一個 5169 字的章節被切成 150 塊、平均只有 34 字，而該章最短的段落只有 2–5 個字（「好。」「嗯。」）——對話密集處等於每兩秒卡一次。

改成直接在整份章節內文上切、允許段落合併，並把長度下限從 40 拉到 110、上限從 160 拉到 280。同一章 **150 塊降到 40 塊**，每章的間隔總時間從 105 秒降到 28 秒。

連帶處理三件事：段落可能跨越多個 `<p>`，所以「正在朗讀」的標記與自動捲動改成跟著詞走；段落變長後只在段落起點回寫閱讀位置會讓自動書籤落後，改成每前進 30 字同步一次；從閱讀位置起播時用段內位移精準接上，否則最多會倒退 280 字（約 40 秒）。

**上下鍵改為換頁**

原本是逐行捲動 120px，改成與 PgUp/PgDn 相同的換頁。逐行捲動用滑鼠滾輪即可。

**語速上限 3x → 4x**

實測 SAPI 的真實上限就是 4：1x 14626ms、2x 10514ms、3x 9424ms、4x 7571ms、**5x 7563ms**（與 4x 相同）。所以開到 4 剛好是天花板，再往上調沒有意義。
